...
function hookInstallCommand(subcommand: string | undefined, options: CliOptions): void {
  ...
  if (options.print) {
    const summary: RippleHookInstallSummary = {
      ...
      overwritten: false,
      content,
      nextSteps: ["Review the hook script, then run ripple hook install to write it."],
    };
    if (options.json) {
      printJson(summary);
    } else {
  ...
  if (existed && !options.force) {
    const summary: RippleHookInstallSummary = {
      ...
      written: false,
      overwritten: false,
      nextSteps: ["Existing pre-commit hook was not overwritten. Re-run with --force to replace it."],
    };
    if (options.json) {
      printJson(summary);
    } else {
      console.log("Ripple pre-commit hook already exists");
      console.log(`Path: ${relativeHookPath}`);
  ...
  if (options.json) {
    ...
    console.log(existed ? "Ripple pre-commit hook overwritten" : "Ripple pre-commit hook installed");
    console.log(`Path: ${relativeHookPath}`);
