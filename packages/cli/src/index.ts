...
function hookInstallCommand(subcommand: string | undefined, options: CliOptions): void {
  ...
  const postCommitHookPath = path.join(workspaceRoot, ".git", "hooks", "post-commit");
  const relativeHookPath = path.join(".git", "hooks", "pre-commit").split(path.sep).join("/");
  const relativePostCommitHookPath = path.join(".git", "hooks", "post-commit").split(path.sep).join("/");
  ...
  const preCommitExisted = fs.existsSync(hookPath);
  const postCommitExisted = fs.existsSync(postCommitHookPath);
  const existed = preCommitExisted || postCommitExisted;
  if (existed && !options.force) {
    const summary: RippleHookInstallSummary = {
      ...
      written: false,
      overwritten: false,
      nextSteps: ["Existing Git hook was not overwritten. Re-run with --force to replace Ripple hooks."],
    };
    if (options.json) {
      printJson(summary);
    } else {
      console.log("Ripple Git hook already exists");
      console.log(`Pre-commit: ${relativeHookPath}`);
      console.log(`Post-commit: ${relativePostCommitHookPath}`);
      console.log("Use --force to overwrite Ripple hooks.");
    }
    return;
  }
  ...
  fs.writeFileSync(postCommitHookPath, postCommitContent, { encoding: "utf8", mode: 0o755 });
  try {
  ...
  if (options.json) {
    ...
    console.log(existed ? "Ripple Git hooks overwritten" : "Ripple Git hooks installed");
    console.log(`Pre-commit: ${relativeHookPath}`);
    console.log(`Post-commit: ${relativePostCommitHookPath}`);
